...
function cliCreateXrpcRelay() {
  return createXrpcRelay({ logger, dispatcherHost: options.relayDispatcherHost, signer: atproto.signer, keypair });
}
...
if (options.computeProviderDigitaloceanToken) {
  const relay = cliCreateXrpcRelay();
...
if (options.computeProviderDenoWorker) {
  ...
  serves.push(serve);
  providers.push(createComputeProviderMarketBidderHooks({
    provider: await createComputeProviderDenoWorker({ logger, atproto, serve, getIssuerUrl: () => didWebToHttps(relay.proxyRef) }),
  }));
}

const relays = [];
if (!options.noXrpcRelay) relays.push(cliCreateXrpcRelay());
...
This is ~1:1 with the `refactor.md` sketch. Differences from sketch are intentional and minor: `cliCreateXrpcRelay()` also takes `signer`/`keypair` (service-auth + nonce need them); `createMarketBidder` internally owns `activeContracts` + the provider `setup`/`teardown`/callback-merge loops (honors the sketch's "just pass `providers` and have it happen" TODO); `proxyRef` read only lazily (server-assigned, decision below).
...
2. **`proxyRef` is dispatcher-assigned**, NOT precomputed client-side. Known only after `createSubscriber` resolves. Read **lazily everywhere** via `() => relay.proxyRef`. The one eager consumer (OIDC `serviceUrl`) runs in a post-connect hook (`serve.onConnected`).
3. **Relay late-binding**: `createXrpcRelay` builds synchronously with empty `proxyRef`; defers the WebSocket connect into async `onServe(fetch)`. `createServe.beginServe()` mounts routes, `await relay.onServe(app.fetch)` (connect → set `proxyRef`), then fires `serve.onConnected(proxyRef)`. Resolves the serve→relay→app→serve cycle.
...
export interface RelayRef {
  proxyRef: string;                                   // "" until connected; read lazily
  onServe(fetch: (req: Request) => Promise<Response>): Promise<void>;  // connects, sets proxyRef
  close(): void;
}
...
export interface ServeHandle {
  app: Hono;
  addRelay(relay: RelayRef): void;                    // attach a relay built after createServe (LocalPDSAgent)
  onConnected(cb: (proxyRef: string) => void | Promise<void>): void;  // fired after relays connect (OIDC mount)
...
### Phase 3 — `createXrpcRelay()` (`atproto-market/lib/xrpc-relay/`)
Synchronous constructor, empty `proxyRef`; `onServe(fetch)` builds `createSubscriberFactory({ app:{fetch} })` then `await createSubscriber({ keypair, getServiceAuthToken, dispatcherHost, handleRequest })`, sets `proxyRef = handle.proxyRef`, `ws`. `getServiceAuthToken=(lxm)=>signServiceAuth(signer,{aud:"did:web:"+hostnameOnly(host),lxm})`. `close()`→`ws?.close()`. **did-key-relay prerequisite (MAYBE)**: if the dispatcher assigns the subdomain, `createSubscriber` must return the server-confirmed `proxyRef` (parse `#registered`, subscriber/mod.ts:103) rather than the client-derived value (:54) — confirm dispatcher behavior. NEW `deno.json` (deps: did-key-relay-subscriber-xrpc, hono-factory-did-key-relay-subscriber-xrpc, did-key-relay-subscriber-abc, did-key-relay-common, logger, hostname-helpers, atproto-repo-deno for `signServiceAuth`). MODIFY `atproto-market/deno.json`: workspace + imports (`xrpc-relay`, `serve`).
...
- `LocalPDSAgent` owns its relay internally (decision #10 ordering): PLC-register keypair→did; `signer={did:()=>did,sign:keypair.sign}`; `createRepoFactory` mounts on `opts.serve.app`; `createXrpcRelay({signer,keypair,...})`; `opts.serve.addRelay(relay)`; exposes `.signer`,`.did`,`.repoApi`,`beginServe()`(=`opts.serve.beginServe()`),`stop()`.
...
- `lib/compute-provider-digitalocean/mod.ts` + `lib/compute-provider-local/mod.ts`: adopt new ctx; pull `getAgentDid`/`createRecord`/`deleteRecord` from `atproto`; mount OIDC via `serve.onConnected(proxyRef => serve.app.route("/", createOidcIssuer({getIssuerUrl, serviceUrl: didWebToHttps(proxyRef), ...}).app))`. Add `@publicdomainrelay/serve` to their deno.json (+ `hono-compute-provider/deno.json` import map).
...
ALWAYS: `createLogger({serviceName})` JSON logger; `createServe()` (composable, `beginServe()`, `shutdown()`→SIGINT/SIGTERM); `beginServe()` resolves after all deps ready; provider array each owning relay+serve+OIDC (OIDC mounted on provider's serve in `serve.onConnected`); `cliCreateXrpcRelay()` (sync, defers connect, `proxyRef` server-assigned/lazy); pass `atproto` instance directly; `createMarketBidder({providers})` owns lifecycle; signal handlers in CLI only. NEVER: raw `Deno.serve` in CLI/lib; wrapping methods in intermediate objects; OIDC bolted on bidder app post-hoc; if-branch single provider; I/O in lib (`Deno.serve`/`addSignalListener`/WS connect/`Deno.env.get`); precompute/eager-read `proxyRef`.

### Phases 10-12 — All other CLIs (per-CLI: logger→`createLogger({serviceName})`, `Deno.serve`→`createServe`, hand-rolled subscriber→`createXrpcRelay`, `shutdown()` wired, no I/O left in lib; `deno check` + `--help` each)
- **Tier 1 (serve-only)**: `typescript-helpers/hono-http-static`, `deno-worker-sandbox/hono-sandbox`, `atproto-relay/hono-atproto-relay`.
- **Tier 2 (relay+serve)**: `did-key-relay/hono-did-key-relay-relayer`, `did-key-relay/hono-did-key-relay-subscriber`, `deno-worker-sandbox/hono-compute-deno`, `hono-pds/main.ts` (keep `createFromEnv`/`start`+`import.meta.main`), `hono-jsr/hono-package-registry`, `hono-compute-provider/hono-compute-provider` (route `killAllDroplets` via teardown).
...
| MAYBE | `did-key-relay/lib/did-key-relay-subscriber-xrpc/mod.ts` (server proxyRef from `#registered`) |
| MODIFY | `CLAUDE.md` |
...
```
Behavior: `beginServe()` resolves only after relay `proxyRef`s set + routes mounted; SIGINT closes all WebSockets, process exits ~1s (no hang); OIDC discovery reachable on each provider's relay endpoint post-connect; rewritten `mod.ts` structurally matches `refactor.md`. Each Tier CLI: `deno check` + `--help` before moving on.
